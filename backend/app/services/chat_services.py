...
class ChatService:
    ...
    async def text_to_speech_base64(self, text: str) -> str:
        """Convert text to speech using gTTS and return base64 audio (async)"""
        try:
            # Clean text for speech (limit length for faster processing)
            clean_text = re.sub(r'[^\w\s.,!?;:()\-@#&+*%/]', '', text)
            clean_text = clean_text[:300]  # Reduced length for faster TTS
            ...
            audio_base64 = await loop.run_in_executor(None, sync_tts)
        ...
        if requested_platform and requested_platform in self.social_media_urls:
            ...
            try:
                ...
                return {
                    ...
                    "audio": "",  # No audio for social media actions
                    "action": "open_url",
                    "url": url,
    ...
    async def get_response(self, message: str, session_id: str = None, use_voice: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if cleaned_message in ['hi', 'hello', 'hey']:
                ...
                audio_base64 = await self.text_to_speech_base64(response_text) if use_voice else ""
                sources = ["greeting"]
                ...
                if context:
                    ...
                    if any(word in cleaned_message for word in ['github', 'git']):
                        ...
                        if 'email' not in cleaned_message:
                            ...
                            audio_base64 = await self.text_to_speech_base64("My contact number is plus ninety one ninety seven sixty seven ninety five twenty four seventy one") if use_voice else ""
                        else:
                            response_text = "My contact information:\nEmail: sandipbaste999@gmail.com\nPhone: +91 9767952471\nGitHub: github.com/sandipbaste\nLinkedIn: linkedin.com/in/sandipbaste999"
                            audio_base64 = await self.text_to_speech_base64("My contact information includes email, phone, GitHub, and LinkedIn profiles.") if use_voice else ""
                    
                    elif any(word in cleaned_message for word in ['project']):
                        response_text = self.format_projects_response(context)
                        audio_base64 = await self.text_to_speech_base64("I have worked on several AI projects including WhatsApp chatbot, voice assistant, and video insight extractor.") if use_voice else ""
                    
                    elif any(word in cleaned_message for word in ['skill']):
                        response_text = self.extract_skills_response(context)
                        audio_base64 = await self.text_to_speech_base64("My skills include Python, Generative AI, Large Language Models, and various AI frameworks.") if use_voice else ""
                        ...
                        audio_base64 = await self.text_to_speech_base64("I worked as an AI/ML Developer Intern at Application Square Infotech, where I developed chatbot and voicebot applications.") if use_voice else ""
                    
                    elif any(word in cleaned_message for word in ['education']):
                        response_text = self.extract_education_response(context)
                        audio_base64 = await self.text_to_speech_base64("I'm pursuing Master's in Computer Science with a CGPA of seven point nine one, and completed Bachelor's with eight point two seven CGPA.") if use_voice else ""
                    
                    elif any(word in cleaned_message for word in ['summary', 'about', 'who is']):
                        response_text = self.extract_summary_response(context)
                        audio_base64 = await self.text_to_speech_base64("I'm Sandip Baste, an AI/ML Developer specializing in Generative AI and Large Language Models. I build intelligent AI solutions.") if use_voice else ""
                    ...
                    audio_base64 = await self.text_to_speech_base64("I couldn't find specific information about that. Please ask about my skills, projects, or experience.") if use_voice else ""
            ...
            if any(greet in cleaned_message for greet in ['hi', 'hello', 'hey']):
                ...
                audio_base64 = await self.text_to_speech_base64(fallback_response) if use_voice else ""
                ...
                audio_base64 = await self.text_to_speech_base64("You can find my GitHub profile at github.com/sandipbaste") if use_voice else ""
            elif any(word in cleaned_message for word in ['email']):
                fallback_response = "You can reach me at: sandipbaste999@gmail.com"
                audio_base64 = await self.text_to_speech_base64("You can reach me at sandipbaste999@gmail.com") if use_voice else ""
            else:
                ...
                audio_base64 = await self.text_to_speech_base64("I'm here to help you learn about Sandip's professional background.") if use_voice else ""
